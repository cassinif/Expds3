...
mrange_etd2rkds = 3000:1000:6000;
mrange_exprk3ds_cplx = 1000:500:2500;
mrange_exprk3ds_real = 1000:500:2500;
mrange_etd2rk = 3000:1000:6000;
mrange_exprk3 = 1000:500:2500;
...
rng('default')
U0{1} = au+av+1e-5*rand(n);
U0{2} = av/((au+av).^2)+1e-5*rand(n);
u0 = [U0{1}(:);U0{2}(:)];

%nstepsref = 6000;
%tau = T/nstepsref;
disp('Computing/loading reference solution...')
%Uref = exprk3ds_real(U0,g,A,nstepsref,tau,c2);
load('ref_SCHNAK_2D.mat','Uref')
...
for m = mrange_etd2rkds
  ...
  tau = T/m;
  tic
  Uetd2rkds = etd2rkds(U0,g,A,m,tau);
...
for m = mrange_exprk3ds_cplx
  ...
  tau = T/m;
  tic
  Uexprk3ds_cplx = exprk3ds_cplx(U0,g,A,m,tau,c2);
...
for m = mrange_exprk3ds_real
  ...
  tau = T/m;
  tic
  Uexprk3ds_real = exprk3ds_real(U0,g,A,m,tau,c2);
...
for m = mrange_etd2rk
  ...
  tau = T/m;
  tic
  Uetd2rk = etd2rk(u0,gvec,Kfun,m,tau,tol_kry);
...
for m = mrange_exprk3
  ...
  tau = T/m;
  tic
  Urk3 = exprk3(u0,gvec,Kfun,m,tau,tol_kry,c2);
